%%processes images compressed at different JPEG quality levels and saves intermediate error features used for training/testing a classifier.
addpath('dependencies/jpeg_read_toolbox/')
addpath('all_needed_matlab_functions/')

dir_path = '../data/'
Q_list = [40,70,75,80,65,90]; %All quality factors reported. 
stability_index = 'all';
...
ld = 1; % Label for double-compressed images
ls = 0; % Label for single-compressed images
prefixes = {'test/','train/'};
patch_size = 8;
...
for q = 2
    ...
    %Patches from single JPEG compression go in .../single/
    ...
    for m = 1:2 %You process both the training and testing data.
        prefix = prefixes{m};
        ...
        for k = 1:4 %These are grouped patches, possibly by stability 
        ...
        for k = 1:3
            ...
            if(strcmp(prefix,'train/'))
               ...
               %trunc: truncation errors (quantization errors)round: rounding errors from JPEG compression single_dct_error: DCT-based distortion single_error: spatial domain error single_EBSF: an engineered feature vector (likely “Error-Based Statistical Features”)
                ...
                if(k==1) %For the first iteration, you start the feature matrices. On later iterations, you may be appending features.
    ...
    %Adds class labels (0 for single, 1 for double) as the last column of feature vectors.
...
%Vertically stacks to form full train/test matrices.
