...
# TensorFlow imports - using proper namespaces
import tensorflow as tf
from tensorflow import set_random_seed
set_random_seed(2)  # This replaces the deprecated set_random_seed
...
# Keras imports (should be imported from tensorflow.keras not tf.keras or standalone keras)
...
parser.add_argument("--Qf", default=40, type=int)
parser.add_argument("--index", default='all', type=str)
...
dir_path = '/home/user/Documents/try/jpeg_dc_detection/data folder/dataset/'
res_prefix = '/home/user/Documents/try/jpeg_dc_detection/proposed_results/'
...
for q in range(len(Q_list)):
    ...
    # Loading Spatial and DCT errors - MODIFIED TO USE NPZ FILES
    for i in range(k_single):
        sp_load = 'error'
        dct_load = 'dct_error'

        # Load NPZ files instead of MAT files
        single_train_error_npz = np.load(single_train_prefix + str(i+1) + '/single_' + str(sp_load) + '.npz')
        double_train_error_npz = np.load(double_train_prefix + str(i+2) + '/double_' + str(sp_load) + '.npz')

        single_test_error_npz = np.load(single_test_prefix + str(i+1) + '/single_' + str(sp_load) + '.npz')
        double_test_error_npz = np.load(double_test_prefix + str(i+2) + '/double_' + str(sp_load) + '.npz')

        single_train_dct_npz = np.load(single_train_prefix + str(i+1) + '/single_' + str(dct_load) + '.npz')
        double_train_dct_npz = np.load(double_train_prefix + str(i+2) + '/double_' + str(dct_load) + '.npz')

        single_test_dct_npz = np.load(single_test_prefix + str(i+1) + '/single_' + str(dct_load) + '.npz')
        double_test_dct_npz = np.load(double_test_prefix + str(i+2) + '/double_' + str(dct_load) + '.npz')

        # Get the key for each npz file (assuming the first key contains our data)
        single_train_key = list(single_train_error_npz.keys())[0]  
        double_train_key = list(double_train_error_npz.keys())[0]
        single_test_key = list(single_test_error_npz.keys())[0]
        double_test_key = list(double_test_error_npz.keys())[0]
        
        single_train_dct_key = list(single_train_dct_npz.keys())[0]
        double_train_dct_key = list(double_train_dct_npz.keys())[0]
        single_test_dct_key = list(single_test_dct_npz.keys())[0]
        double_test_dct_key = list(double_test_dct_npz.keys())[0]

        # Append data to respective lists
        single_train_data.append(single_train_error_npz[single_train_key].reshape(-1, 1, 8, 8))
        double_train_data.append(double_train_error_npz[double_train_key].reshape(-1, 1, 8, 8))

        single_test_data.append(single_test_error_npz[single_test_key].reshape(-1, 1, 8, 8))
        double_test_data.append(double_test_error_npz[double_test_key].reshape(-1, 1, 8, 8))

        single_train_dct_data.append(single_train_dct_npz[single_train_dct_key].reshape(-1, 1, 8, 8))
        double_train_dct_data.append(double_train_dct_npz[double_train_dct_key].reshape(-1, 1, 8, 8))

        single_test_dct_data.append(single_test_dct_npz[single_test_dct_key].reshape(-1, 1, 8, 8))
        double_test_dct_data.append(double_test_dct_npz[double_test_dct_key].reshape(-1, 1, 8, 8))
    ...
    for r in range(runs):
        ...
        if(ptr == 0):
            ...
            # Fit CNN with model.
            history = CNN.fit([x_train, x_train_dct], y_train, epochs=max_epochs, batch_size=bs, shuffle=False, 
                              validation_data=([x_valid, x_valid_dct], y_valid), callbacks=callbacks_list)

            # Saving validation plots.
            plt.figure()
            plt.plot(history.history['acc'])  # Updated from 'acc'
            plt.plot(history.history['val_acc'])  # Updated from 'val_acc'
        ...
        # Saving individual iterations - MODIFIED TO SAVE NPZ INSTEAD OF MAT
        np.savez(result_path + '/runs/' + save_suffix + '/results/results_itr_' + str(r) + '.npz', 
                AR=(TP+TN)/2, 
                TP=TP, 
                TN=TN, 
                y_pred=y_pred, 
    ...
    # Saving the average results for quality factor Qf - MODIFIED TO SAVE NPZ INSTEAD OF MAT
    np.savez(result_path + '/runs/' + save_suffix + '/results/avg.npz', 
            AR=AR_mean, 
            TP=TP_mean, 
            TN=TN_mean, 
            single_test_size=single_size, 
            double_test_size=double_size)
...
for q in range(len(Q_list)):
    ...
    for r in range(runs):
        ...
        if(ptr == 0):
            CNN.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=lr), loss='binary_crossentropy', metrics=['accuracy', custom])
            mkdir(res_prefix)
            mkdir(res_prefix + save_name)
            ...
            # Saving validation plots.
            plt.figure()
            plt.plot(history.history['accuracy'])  # Updated from 'acc'
            plt.plot(history.history['val_accuracy'])  # Updated from 'val_acc'
            plt.title('model accuracy')
            plt.ylabel('accuracy')
            plt.xlabel('epoch')
            plt.legend(['train', 'test'], loc='upper left')
